...
A [square-free word](https://en.wikipedia.org/wiki/Square-free_word) is one that does not repeat patterns contiguously. For example, *aba* is OK because the *a*s are separated, but *abab* is not because the *ab*s are contiguous. Also, the square need not use up the entire word. For example, *caac* has a square *aa*, but neither *c* is part of it.

The program in this repository transforms a *specific* input string that uses $$k$$ characters (e.g. *abc* uses 3) into a *square-free* string with *n* characters.
...
[^***]: Depending on the topic, words may also be infinite. Let's focus on finite ones.
